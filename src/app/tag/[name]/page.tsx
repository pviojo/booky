    ...
    <div className='inner'>
      <div className='px-5 md:px-10'>
        <nav className='flex mb-2 ' aria-label='Breadcrumb'>
          <ol className='inline-flex items-center space-x-1 md:space-x-3'>
            <li className='inline-flex items-center'>
              ...
              <svg
                className='w-3 h-2 mx-1 text-gray-400'
                aria-hidden='true'
        ...
        <div className='page-title'>
          <FontAwesomeIcon icon={faTag} className='mr-2 text-2xl' />
          {tagName}
        </div>
        {isAuthenticated && (
          <FormBookmark authorId={AUTHOR_ID} defaultTags={tagName} />
        )}
        <div className='mt-10'>
          {bookmarks?.length ? (
            <>
              {bookmarks.map((bookmark, i) => (
                ...
                >
                  <div
                    ...
                    className='items-center justify-center mt-1 text-sm bg-gray-200 rounded-full aspect-square dark:bg-gray-500 grid'
                  >
                    <span>{i + 1}</span>
                  </div>
                  <div className='flex-auto' id='1'>
                    <Bookmark
                      bookmark={bookmark}
